...
from simulate_c_bssn_driver import adm_c_to_bssn_state, advance_rk4_c, c_bssn_rhs
from simulate_b_bssn_driver import state_metrics
...
def main() -> None:
    parser = argparse.ArgumentParser(description="C 支 BSSN 型长时间溢出诊断")
    parser.add_argument("--output", type=str, default="visualizations/c_bssn_overflow_diagnosis")
    ...
    _, c_adm = make_bc_initial_states(
        rho0, s0, omega_init, 1.0, 300.0, args.ell, dx, dz,
        c_phi_mode=args.c_phi_mode, elliptic_lambda=args.elliptic_lambda,
    )
    state = adm_c_to_bssn_state(c_adm)
    ...
    evolve_keys = (
        ...
        "at_xx", "at_xz", "at_zz", "lapse", "shift_x", "shift_z",
        "n", "s", "phi", "pi_phi",
    )
            ...
            for step in range(1, args.steps + 1):
                ...
                if step % args.checkpoint_every == 0 or step == 1:
                    rec = {
                        ...
                        "t": t,
                        **state_metrics(state),
                        "phi_minus_1_max": float(np.max(np.abs(state["phi"] - 1.0))),
                        "pi_phi_abs_max": float(np.max(np.abs(state["pi_phi"]))),
