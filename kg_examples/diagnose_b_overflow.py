...
def main() -> None:
    parser = argparse.ArgumentParser(description="定位 B(flat) 长时间演化中的首次溢出或非有限值")
    parser.add_argument("--output", type=str, default="visualizations/b_overflow_diagnosis", help="输出目录")
    parser.add_argument("--checkpoint-every", type=int, default=1000, help="检查点写入步数间隔")
    parser.add_argument("--gauge-mode", choices=["synchronous", "1plog"], default="1plog", help="几何切片规范")
    parser.add_argument("--steps", type=int, default=64000, help="步数")
    parser.add_argument("--dt", type=float, default=2.5e-4, help="时间步长")
    parser.add_argument("--finite-check-every", type=int, default=100, help="有限性检查间隔")
        ...
        try:
            for step in range(1, args.steps + 1):
                ...
                if step % args.checkpoint_every == 0 or step == 1:
                    ...
                    f.flush()
            result = {"status": "completed", "step": args.steps, "t": t, "b": state_metrics(b_state)}
